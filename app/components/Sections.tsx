          ...
          <Stack gap="sm">
            ...
            <Group gap="xs">
              {socialLinks.map(({ href, label, icon: Icon }) => (
                <ActionIcon key={href} variant="light" size="lg" component="a" href={href} target="_blank" rel="noreferrer" aria-label={label}>
                  <Icon size={18} />
                </ActionIcon>
              ))}
            </Group>
          ...
          <Paper withBorder p="lg" radius="md" className="contact-card">
            <form ref={formRef} onSubmit={onSubmit}>
              <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
                <TextInput name="name" required label={text.contact.fields.name} placeholder={text.contact.fields.name} />
                <TextInput name="company" label={text.contact.fields.company} placeholder={text.contact.fields.company} />
              </SimpleGrid>
              <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md" mt="md">
                <TextInput name="email" type="email" required label={text.contact.fields.email} placeholder="email@ejemplo.com" />
                <TextInput name="phone" label="Teléfono (opcional)" placeholder="+54..." />
              </SimpleGrid>
              <Textarea name="message" label={text.contact.fields.message} placeholder={text.contact.fields.message} minRows={4} mt="md" />
              <Group gap="sm" mt="md">
                <Button type="submit" variant="gradient" rightSection={<IconSend size={16} />}>
                  {text.contact.submit}
                </Button>
                <Button type="button" variant="outline" color="teal" rightSection={<IconBrandWhatsapp size={16} />} onClick={onWhatsapp}>
